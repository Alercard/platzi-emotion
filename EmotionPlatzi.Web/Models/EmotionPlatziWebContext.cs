...
namespace EmotionPlatzi.Web.Models
{
    public class EmotionPlatziWebContext : DbContext
    {
        ...
        {
            /*
            // por cada cambio en el modelo, se recreara la base de datos
            // esto solo aplica en ambientes de desarrollo
            */
            Database.SetInitializer<EmotionPlatziWebContext>(
                new DropCreateDatabaseIfModelChanges<EmotionPlatziWebContext>()
            );
            /*
             * Para ambientes de produccion. Esto necesitara mas programacion de como aplicar los cambios
            Database.SetInitializer<EmotionPlatziWebContext>(
                new MigrateDatabaseToLatestVersion<EmotionPlatziWebContext>()
            );
             */
        }
